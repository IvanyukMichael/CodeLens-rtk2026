...
### Запуск через Docker (альтернативный путь)

Запуск двумя командами выше — основной. Docker — альтернатива «одной командой»: образ
сам строит базу и поднимает UI. Контейнер **CPU-only** (GPU намеренно не пробрасывается —
ради простоты и предсказуемости на демо).

```bash
docker compose up --build                       # режим по умолчанию: ВСЯ база (168 файлов, ≥80 ТЗ)
INDEX_SCOPE=gymhero docker compose up --build   # быстрый/официальный режим: только gymhero
# открыть http://localhost:8501
```

- **Два режима индексации** (применяются при первом старте, пока том базы пуст):
  - **по умолчанию** (`INDEX_SCOPE=full`) — индексирует всю базу: `gymhero + corpus_extra +
    corpus_polyglot` = 168 файлов кода (демонстрирует требование «база ≥80 файлов»);
  - `INDEX_SCOPE=gymhero` — только официальная коллекция `codelens_gymhero` (быстрее).
  - **Официальный P@5=0.800 в обоих режимах считается строго по `codelens_gymhero`** —
    он не зависит от того, проиндексирована вся база или только gymhero.
- **Тайминги (замерено на живом контейнере).** Холодный старт ≈ 7 мин — целиком разовые
  загрузки (образ Ollama + bge-m3 ~2.3 ГБ), выполняются один раз. Сама индексация — **2.6 с**
  (полная база) / **0.9 с** (gymhero) благодаря запечённому кэшу эмбеддингов
  (`cache/index_embeddings.pkl`, 100% покрытие): модель для эмбеддинга не загружается. Каждый
  следующий `up` ≈ **4 с** (volumes `hf_cache`/`chroma_db` persist). CPU-режим не стоит ничего
  для ретрива и индексации; медленна только живая HyDE-генерация на новых запросах (~22 с на
  CPU), поэтому официальные 15 идут из кэша, а GPU остаётся опциональным.
- **Идемпотентность:** если база уже построена (том `chroma_db`), индексация пропускается.
  Чтобы пересобрать в другом режиме — очистите тома: `docker compose down -v`.
- **HyDE/чат — опциональны.** Ollama поднимается отдельным сервисом, но модель не
  подтягивается автоматически. Чтобы включить полноценный HyDE/чат, после старта выполните
  команду ниже. Без неё поиск **не падает** — `search.py` делает graceful fallback (HyDE из
  кэша / чат показывает найденные фрагменты).

```bash
docker compose exec ollama ollama pull qwen2.5-coder:7b
```

> ⚠️ **Не путайте с `gymhero/docker-compose.yaml`** — это Docker исходного приложения gymhero
> (FastAPI + PostgreSQL, порт 8000), которое мы лишь индексируем. К продукту CodeLens он не
> относится; для запуска используйте корневой `docker-compose.yml` (порт 8501).
...
Исследовали 5 рычагов; принимаем только те, что согласованы **и на официальных 15 (знак истины),
и на combined-269 (мощность)**. **Принятые** — enrichment (+0.111 на офиц.15) и HyDE-mix
(0.767 → 0.800), см. §5. **Отклонённые** — ниже, в том же формате, что страница «Результаты» в
UI и [REPORT §3.4](REPORT.md) (там — доверительные интервалы и paired-bootstrap). Ведущая колонка —
...
Δ — относительно базы каждой техники.
...
BM25-гибрид отклонён по другой причине: вес подобран на 15 вопросах (оверфит), на held-out он ниже
чистого dense. Multi-HyDE не даёт прироста ни на одном наборе. Итог: ни одна техника не превосходит
bge-m3 + enriched + HyDE-mix на человеческой метрике (0.800); реранкинг отклонён как класс.
...
таблицу не сводится (отсутствие прогона, не результат); прод остаётся на воспроизводимой локальной
bge-m3 — без API-ключей.
